...
module ahb_top_tb;
    ...
    initial begin
        ...
        // ==========================================================
        // PART 1: WRAP16 BYTE BURST WRITE (16 Beats, Hsize = 3'b000)
        // Starting in the middle of the 16-byte boundary (0x18) 
        // to clearly demonstrate wrap-around behavior within [0x10 : 0x1F]
        // ==========================================================

        // Beat 1: Address Phase (NONSEQ) - Middle start address
        @(posedge Hclk);
        #1;
        Haddr_m  = 32'h0000_0018; // Middle start address (Boundary block: 0x10 to 0x1F)
        Hwdata_m = 32'h0000_0001; // Data for beat 1
        Hwrite_m = 1'b1;          // Write mode
        Htrans_m = 2'b10;         // NONSEQ
        Hburst_m = 3'b110;        // WRAP16 (3'b110)
        Hsize_m  = 3'b000;        // Byte (8-bit)

        // Beats 2 to 16: Address Phase (SEQ) + Data Phases (Wraps from 0x1F back to 0x10)
        ...
        // ==========================================================
        // PART 2: WRAP16 BYTE BURST READ (16 Beats, Hsize = 3'b000)
        // ==========================================================

        // Beat 1: Address Phase (NONSEQ) - Same middle start address
        @(posedge Hclk);
        #1;
        Haddr_m  = 32'h0000_0018; // Same middle start address
